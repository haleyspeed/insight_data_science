...
st.sidebar.subheader('Estimate Return on Investment')
st.sidebar.title( ' ')
fig, ax = plt.subplots()
...
server_name = st.sidebar.text_input("", max_chars = 100)
st.write('')
...
st.sidebar.subheader('Curated Recommendations: ')

recs = get_recommendations(df_rec)
for rec in recs:
    st.sidebar.subheader(rec[1])
    st.sidebar.markdown(rec[2])
    st.sidebar.markdown(' ')

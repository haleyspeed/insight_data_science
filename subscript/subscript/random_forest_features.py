...
lapsed_achievements = cf.random_forest_feature_selection (lapsed, 'lapsed')


dfo = pd.DataFrame([sub_achievements,lapsed_achievements]).T
dfo.columns = ['current', 'lapsed']
dfo['completed_in_lapsed'] = np.nan
dfo['target_content'] = ''
for index, row in dfo.iterrows():
    if row.current not in dfo.lapsed.values:
        dfo.at[index,'target_content'] = row.current
        dfo.at[index,'completed_in_lapsed'] = 0
    elif row.current in dfo.lapsed.values:
        dfo.at[index, 'target_content'] = ''
        dfo.at[index,'completed_in_lapsed'] = 1
dfo.sort_values('completed_in_lapsed').to_csv(os.path.join(cn.clean_dir, 'pickles', 'feature_selection_' + str(risk) + '-' + 'lapsed.csv'))
print('Target Content: ')
print(dfo.current[dfo.completed_in_lapsed == 0])
